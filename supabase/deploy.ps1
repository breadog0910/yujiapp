...
Write-Info ""
Write-Info "【下一步 - 数据库迁移（二选一）】"
Write-Info "方式 A（推荐，可重复/可回滚）：在本脚本执行前或之后运行"
Write-Info "   supabase db push"
Write-Info "   它会自动应用 supabase/migrations/ 下唯一有效的迁移文件:"
Write-Info "   - 999_all_in_one.sql（已整合 001-007：schema + RLS + 种子 + 农场 + 多地块 + 约束，完全幂等）"
Write-Info "   （001-006 已归档到 supabase/migrations/_legacy/，不会被 CLI 应用）"
Write-Info ""
Write-Info "方式 B（纯手动）：Supabase Dashboard → SQL Editor 粘贴并执行"
Write-Info "   supabase/migrations/999_all_in_one.sql → Run"
Write-Info ""
Write-Info "【下一步 - Secrets 配置】"
Write-Info "1. 在 Dashboard → Edge Functions → Secrets 中添加:"
...
Write-Info ""
Write-Info "2. 修改 yuji-app/index.html 中的 SUPABASE_URL 和 SUPABASE_ANON_KEY"
Write-Info ""
Write-Info "3. 注册第一个账号后，在 Dashboard → Table Editor → users 表"
