...
> 予己处理的是用户最私密的数据——情绪、日记、自我认知。所以隐私保护是**底线承诺**。

| 维度           | 承诺与实现                                                                                                   |
| ------------ | ------------------------------------------------------------------------------------------------------- |
| 🗄️ **数据隔离** | 所有表启用 PostgreSQL **RLS 行级安全**：普通用户只读公开数据、**仅能读写自己的 user_state**，他人数据在数据库层面就不可见                          |
| 🤖 **AI 隐私** | AI 仅在关键环节按需携带必要上下文（说明书总结 / 树洞引导 / 星点挖掘），**不泄露用户原始文本记录**；System Prompt 约束"温柔的自我观察者"，不做诊断、不做评价，有隐私保护的授权按钮 |
...
- **隐私保护**：详见上文「隐私与安全」章节——RLS 数据隔离 / AI 不泄原文。
